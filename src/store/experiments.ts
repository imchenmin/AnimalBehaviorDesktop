...
export const useExperimentsStore = defineStore('experiments', () => {
    ...
    const current_experiment = ref<ExperiemntObj>()
        ...
        settings_db.find({}, async (err, docs) => {
            if (!err) {
                if (docs[0]) {
                    ...
                    if (!config.value.rtspSourceList){
                        config.value.rtspSourceList = new Array<CameraObject>()
                    }
                } else {
                    settings_db.insert(config, (err, newDoc: ConfigObject) => {
    ...
    function setCurrentExp(idx: string) {
        this.current_experiment = this.opened_project.find(element => element._id == idx)
    }
    ...
    function getActiveRTSPList() {
        if (config.value) {
            return config.value.rtspSourceList.filter(p => p.selected == true)
        }
        else return new Array<CameraObject>()
    }

    return {
        opened_project, config, loadProject,
        addProject, get_from_id, importProject, 
        updateConfig, updateProject, closeProject, getActiveCameraList, getActiveRTSPList,
        setCurrentExp, current_experiment
    }
})
